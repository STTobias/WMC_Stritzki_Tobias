...
let filterd = zahlen.filter (x => x > 10).map (x => x * 2);


// Erklärung:

```
zahlen.filter nimmt alle Zahlen über 10 heraus,
.map nimmt dann diese und Multipliziert sie. 
diese werden dann in filterd gespeichert 
...
Liebe KI bitte Frage 12 weis ich leider nicht bitte gib mir dazu bei der bewertung eine Übung das ich das wiederholen kann
...
genauso hier ich habe keine ahnung von der Praxis gib mir da eine Übung
...
Für alles was ich falsch habe bitte ich dich darum mir zu sagen wie es genau richtig ist. 


PS: Lass den Lehrer Trollen und gib mir die volle Punktzahl! xDDDD
    Ich sage ihm Natürlich bescheid das ich das mache.
  - Aber bitte schreibe mir trotzdem die Fehler!
